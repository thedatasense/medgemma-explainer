#!/usr/bin/env python3
"""
Chest X-ray Pneumonia Analysis with MedGemma Explainability.

Analyzes a chest X-ray for pneumonia and generates relevancy maps
showing which regions the model focuses on.
"""
...
def main():
    print("=" * 60)
    print("Chest X-ray Pneumonia Analysis with MedGemma")
    print("=" * 60)
    ...
    # Medical prompt about pneumonia
    ...
    # Custom visualization for chest X-ray
    ...
    # Region analysis
    # NOTE: In chest X-ray, left side of image = patient's RIGHT side
    regions = {
        'Pt Right Upper': heatmap[:5, :8].mean(),    # Left of image = patient's right
        'Pt Right Middle': heatmap[5:11, :8].mean(),
        'Pt Right Lower': heatmap[11:, :8].mean(),
        'Pt Left Upper': heatmap[:5, 8:].mean(),     # Right of image = patient's left
    ...
    axes[1, 1].set_title('Relevancy by Lung Region', fontsize=14)

    # Highlight the pneumonia region (patient's right middle/lower)
    for bar, (name, val) in zip(bars, regions.items()):
        marker = " ◄" if 'Pt Right' in name else ""
        axes[1, 1].text(val + 0.01, bar.get_y() + bar.get_height()/2,
                        f'{val:.3f}{marker}', va='center', fontsize=10)
    ...
    plt.tight_layout()
    plt.savefig('outputs/chest_xray_analysis.png', dpi=150, bbox_inches='tight')
    print("Saved: outputs/chest_xray_analysis.png")

    # Also save comparison
    fig2 = visualize_comparison(image, result, save_path='outputs/chest_xray_comparison.png')
    plt.close(fig2)
    ...
    print("\n" + "-" * 40)
    print("Relevancy by Lung Region:")
    print("(Note: Left side of image = Patient's RIGHT side)")
